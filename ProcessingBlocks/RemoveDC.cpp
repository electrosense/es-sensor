...
void RemoveDC::run() {
    ...
    while (mRunning) {
        ...
        if (mQueueIn && mQueueIn->try_dequeue(segment)) {
            ...
            //std::transform(iq_complex.begin(), iq_complex.end(),
            iq_complex.begin(),
            //               [](std::complex<double> c) -> std::complex<double>
            { std::complex<double>(real(c)-mean_real,imag(c)-mean_imag); } );

            // Check the performance between std::transform and std::for_each

            std::for_each(iq_complex.begin(), iq_complex.end(), [mean_real,
            mean_imag](std::complex<double> c) { c.real(c.real() - mean_real);
                c.imag(c.imag() - mean_imag);
            } );
            */
            /*
                        std::cout << "RemoveDC ****************" << std::endl;
                        for (unsigned int p = 0; p < iq_complex.size(); p = p +
               1) { std::complex<float> aux = iq_complex[p]; std::cout <<
               aux.real() << "," << aux.imag() << ",";
                        }
                        std::cout << "RemoveDC ****************" << std::endl;
            */
            for (unsigned int p = 0; p < iq_complex.size(); p = p + 1) {
                ...
                iq_complex[p] = std::complex<float>(
                    (aux.real() - 127.4) / 128.0, (aux.imag() - 127.4) / 128.0);
            }
