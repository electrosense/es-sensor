  ...
  while (mRunning) {
    ...
    if (mQueueIn && mQueueIn->try_dequeue(segment)) {
      ...
      for (uint32_t t = 0; t < signal_len; t = t + 2) {
        signal[t / 2] = std::complex<float>(((buf[t] - 127.4) / 128.0),
                                            ((buf[t + 1] - 127.4) / 128.0));
        signal[t / 2] = signal[t / 2] * std::sqrt((float)(signal_len / 2.0) / W);
      }

      // FFT
      fftplan q_f = fft_create_plan(signal_len / 2, signal, signal_freq,
      ...
      uint64_t center_freq = ElectrosenseContext::getInstance()->getMinFreq();
      ...
      memcpy(buffer + sizeof(id) + sizeof(minFreq) + sizeof(maxFreq), &data_len,
             sizeof(data_len));

      // Power data
      memcpy(buffer + sizeof(id) + sizeof(minFreq) + sizeof(maxFreq) +
                 sizeof(data_len),
             &power, sizeof(power));

      unsigned int n =
          sendto(mSocket, (const int *)buffer, buffer_len, MSG_CONFIRM,
      ...
      int status = recvfrom(mSocket, buffer2, 65536, 0,
                            (struct sockaddr *)&saddr, &socklen);

      /**CHANGE IT!!! ***/
      // continue;

      // Read JSON from the data channel connected to WebRTC. It uses same UDP
      // port that the PSD data
      //
      if (status > 0) {
        ...
        ElectrosenseContext::getInstance()->setMinFreq(freq);
        ...
        ElectrosenseContext::getInstance()->setDecoder(decoder);
        ...
        ElectrosenseContext::getInstance()->setGain(g_rtl);
        ...
        if (json_settings != NULL) {
          ...
          ElectrosenseContext::getInstance()->setGainDecoder(gain);
