...
static void tls_info(TLS_Connection *c, const int err, const char *msg) {
    ...
    fprintf(
        stderr, "%s\tTLS %s:%d\t%s:%d\n", err ? "[ERROR]" : "[OK]", msg, err, hostaddr,
        portnum);
    fprintf(stderr, "\t\t%s\n", err ? strerror(errno) : "");
}
...
int tls_init(
    ...
    *tls_c = (TLS_Connection *)malloc(sizeof(TLS_Connection));
    ...
    if (server_mode) {
        ...
        if (tcp_init(&((*tls_c)->tcp_c), hostaddr, portnum) < 0)
            return -1;
    }
    // Initialize SSL/TLS context
    (*tls_c)->ssl_ctx = SSL_CTX_new(method);
    if ((*tls_c)->ssl_ctx == NULL)
        return -2;
    ...
    if (SSL_CTX_use_certificate_file((*tls_c)->ssl_ctx, cert_file, SSL_FILETYPE_PEM) <= 0)
        return -3;
    if (SSL_CTX_use_PrivateKey_file((*tls_c)->ssl_ctx, key_file, SSL_FILETYPE_PEM) <= 0)
        return -4;
    if (SSL_CTX_check_private_key((*tls_c)->ssl_ctx) <= 0)
        return -5;
    // Load trusted CA certificate
    if (SSL_CTX_load_verify_locations((*tls_c)->ssl_ctx, ca_cert_file, NULL) <= 0)
        return -6;
...
int tls_init_p(
    ...
    const char *hostaddr, const int portnum) {
    int err = tls_init(
        tls_c, tcp_c, method, ca_cert_file, cert_file, key_file, hostaddr, portnum);
...
int tls_accept(TLS_Connection *c) {
    ...
    if (SSL_set_fd(c->ssl, c->tcp_c->socket_fd) <= 0)
        return -2;
...
//   if(!X509_LOOKUP_load_file(lookup, ca_cert_file, X509_FILETYPE_PEM)) return
//   -2; lookup = X509_STORE_add_lookup(store, X509_LOOKUP_hash_dir());
