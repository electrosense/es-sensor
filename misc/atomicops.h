...
// Provides portable (VC++2010+, Intel ICC 13, GCC 4.7+, and anything C++11
// compliant) implementation of low-level memory barriers, plus a few
// semi-portable utility macros (for inlining and alignment). Also has a basic
...
#pragma warning(push)
#pragma warning( \
    disable : 4365) // Disable erroneous 'conversion from long to unsigned int,
...
// Provides basic support for atomic variables -- no memory ordering guarantees
    ...
    AE_FORCEINLINE T fetch_add_acquire(T increment) {
        ...
        if (sizeof(T) == 4)
            ...
            return _InterlockedExchangeAdd64(
                (long long volatile *)&value, (long long)increment);
#endif
#else
...
__declspec(dllimport) void *__stdcall CreateSemaphoreW(
    _SECURITY_ATTRIBUTES *lpSemaphoreAttributes, long lInitialCount, long lMaximumCount,
  ...
  private:
    ...
    bool waitWithPartialSpinning(std::int64_t timeout_usecs = -1) {
        ...
        while (true) {
            ...
            if (oldCount < 0)
                return false; // successfully restored things to the way they
                              // were
            // Oh, the producer thread just signaled the semaphore after all.
            // Try again:
            oldCount = m_count.fetch_add_acquire(-1);
