...
int tcp_init(TCP_Connection **c, const char *hostaddr, const int portnum) {
    *c = (TCP_Connection *)malloc(sizeof(TCP_Connection));
    ...
    if (hostaddr == NULL) {
        ...
        if (bind(
                (*c)->socket_fd, (struct sockaddr *)&((*c)->host_addr),
                sizeof((*c)->host_addr)) < 0) {
        ...
        bcopy(
            (char *)hent->h_addr, (char *)&((*c)->host_addr).sin_addr.s_addr,
            hent->h_length);
    }
...
int tcp_connect(TCP_Connection *c) {
    if (connect(c->socket_fd, (struct sockaddr *)&(c->host_addr), sizeof(c->host_addr)) <
        0) {
...
int tcp_accept(TCP_Connection *wel, TCP_Connection **cli) {
    *cli = (TCP_Connection *)malloc(sizeof(TCP_Connection));
    ...
    (*cli)->socket_fd =
        accept(wel->socket_fd, (struct sockaddr *)&((*cli)->host_addr), &len);
    if ((*cli)->socket_fd < 0) {
...
int tcp_read(TCP_Connection *c, void *msg, int len) {
    ...
    while (cnt < 5) {
        b = recvfrom(
            c->socket_fd, msg + bytes_read, len - bytes_read, 0, (struct sockaddr *)0,
            (socklen_t *)0);
...
int tcp_write(TCP_Connection *c, const void *msg, int len) {
    ...
    while (1) {
        ...
        b = write(
            c->socket_fd, msg + bytes_written,
            MIN(DEFAULT_PKT_SIZE_B, len - bytes_written));
...
int tcp_disconnect(TCP_Connection *c) {
    if (c == NULL || c->socket_fd <= 0)
        return 0;
    if (close(c->socket_fd) < 0)
        return -1;
    return 0;
}
