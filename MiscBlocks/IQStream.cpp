...
void IQStream::run() {
    ...
    const char *name_decoders[num_decoders] = { "fm_radio", "am_radio", "dump1090",
                                          "rtl_ais",  "acarsdec", "LTECELL_scanner" };
    ...
    if (pid == 0) {
        switch (OpenRFSenseContext::getInstance()->getDecoder()) {
        case 1:
            ...
            execl(
                "/usr/share/electrosense/decoders/radio.py", "radio.py", "fm", "240000",
                NULL);
        ...
        case 2:
            ...
            execl(
                "/usr/share/electrosense/decoders/radio.py", "radio.py", "am", "60000",
                NULL);
            break;
        ...
        case 3:
            ...
            execl(
                "/usr/share/electrosense/decoders/dump1090", "dump1090", "--esense",
                SOCKET_NAME, NULL);
            break;

        case 4:
            printf("[IQStream] Running decoder: AIS\n");
            execl("/usr/share/electrosense/decoders/ais_decoder", "ais_decoder", NULL);
        ...
        case 5:
            ...
            execl(
                "/usr/share/electrosense/decoders/acarsdec.rtl", "acarsdec.rtl", "-e",
                SOCKET_NAME, NULL);
            break;

        case 6:
            printf("[IQStream] Running decoder: CellSearch\n");
            execl("/usr/share/electrosense/decoders/CellSearch", "CellSearch", NULL);
            break;
        }
        ...
        printf(
            "After the fork!!, the process identifier (pid) "
        ...
        if (OpenRFSenseContext::getInstance()->getDecoder() != 0) {

            cfg.rational =
                rational_dec[OpenRFSenseContext::getInstance()->getDecoder() - 1];
            cfg.inputRate = 2.4e6;
            cfg.outputRate =
                fs_output[OpenRFSenseContext::getInstance()->getDecoder() - 1];
            cfg.transitionWidthFraction =
                transition[OpenRFSenseContext::getInstance()->getDecoder() - 1];
            cfg.desiredRipple =
                ripple[OpenRFSenseContext::getInstance()->getDecoder() - 1];
            cfg.cutoffFraction =
                cutoff[OpenRFSenseContext::getInstance()->getDecoder() - 1];
        ...
        while (mRunning) {
            ...
            if (mQueueIn && mQueueIn->try_dequeue(segment)) {
                if (mSocketEnable) {
                    ...
                    if (OpenRFSenseContext::getInstance()->getDecoder() == 3) {

                        ssize_t ret = sendIQSamples(
                            m_cfd, OpenRFSenseContext::getInstance()->getMinFreq(),
                            OpenRFSenseContext::getInstance()->getGainDecoder(),
                            segment->get_buffer_len(),
                            reinterpret_cast<const int8_t *>(segment->get_buffer()));

                        if (ret < 0 && mRunning) {
                            std::cerr << "[Warning] Error while writing in the "
                                         "socket of the child"
                                      << std::endl;
...
ssize_t IQStream::sendIQSamples(
